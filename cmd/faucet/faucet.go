...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s ALYXs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
func authFacebook(url string) (string, string, common.Address, error) {
	// Ensure the user specified a meaningful URL, no fancy nonsense
	parts := strings.Split(url, "/")
	// https://www.facebook.com/permalink.php?story_fbid=pfbid02LyjEegZgtQWbEKipFintbEBTkb1yrXbezJVt2K2L28M5KmgryKjKNDEWshb9tZexl&id=100085265780451
	if len(parts) < 4 || parts[len(parts)-2] != "posts" {
		if !strings.Contains(url, "story_fbid") || !strings.Contains(url, "id") {
